...
namespace PatchSeller.Web.Schema
{
    ...
    {
        public class CreateUpdate : AbstractValidator<Staff>
        {
            public CreateUpdate()
            {
            ...
            {
                var result = await ValidateAsync(ValidationContext<Staff>.CreateWithOptions((Staff)model, x => x.IncludeProperties(propertyName)));
                if (result.IsValid)
